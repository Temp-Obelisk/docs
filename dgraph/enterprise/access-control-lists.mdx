...
<Note>
  ...
  [v1.1.0]({variables.repos.dgraph}/releases/tag/v1.1.0). The
...
[dgo]({variables.repos.dgo}) or
[dgraph4j]({variables.repos.dgraph}4j), must authenticate with a
...
- **Go** ([dgo client]({variables.repos.dgo})): example
  `acl_over_tls_test.go`
  ([here]({variables.repos.dgraph}/blob/main/tlstest/acl/acl_over_tls_test.go))
- **Java** ([dgraph4j]({variables.repos.dgraph}4j)): example
  `AclTest.java`
  ([here]({variables.repos.dgraph}4j/blob/main/src/test/java/io/dgraph/AclTest.java))
