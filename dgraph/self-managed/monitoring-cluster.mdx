...
1. Create a `YAML` file named `dgraph-prometheus-operator.yaml` and edit the
   ...
   information see,
   [Dgraph Helm chart values]({variables.repos.dgraph}/tree/master/contrib/config/monitoring/prometheus/chart-values).
...
10. In the **Dashboards/Import dashboard** page copy the contents of the
    [`dgraph-kubernetes-grafana-dashboard.json`]({variables.repos.dgraph}/blob/master/contrib/config/monitoring/grafana/dgraph-kubernetes-grafana-dashboard.json)
...
[dgraph-ha.yaml]({variables.repos.dgraph}/tree/master/contrib/config/kubernetes/dgraph-ha/dgraph-ha.yaml)
to learn more.
