...
<Tip>
  ...
  here]({variables.repos.dgraph4j}). Follow the [install
  instructions]({variables.repos.dgraph4j}#download) to get it up
  and running.
</Tip>
...
[this link]({variables.repos.dgraph4j}#supported-versions).

## Quickstart

Build and run the
[DgraphJavaSample]({variables.repos.dgraph4j}/tree/main/samples/DgraphJavaSample)
...
[README]({variables.repos.dgraph4j}/tree/main/samples/DgraphJavaSample/README.md)
of that project.
...
[this issue]({variables.repos.dgraph4j}/issues/108) for more
details).
...
[dgraph4j repo]({variables.repos.dgraph4j}). This particular
