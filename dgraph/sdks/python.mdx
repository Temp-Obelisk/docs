...
<Tip>
  ...
  here]({variables.repos.pydgraph}). Follow the [install
  instructions]({variables.repos.pydgraph}#install) to get it up
  and running.
</Tip>
...
[this link]({variables.repos.pydgraph}#supported-versions).

## Using a client

<Tip>
  You can get a [simple
  example]({variables.repos.pydgraph}/tree/main/examples/simple)
